...
const Logo = "/assets/FC-Logo-Resized-300x76.png";
  ...
  const renderDropdown = (label, items) => {
    ...
    return (
      <div className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
        ...
        <div
          className={`absolute top-full left-0 mt-2 bg-white text-black shadow-lg z-50 min-w-[200px] border transition-all duration-200 ${
            dropdown === label
          ...
          <div className="pt-1">
            {items.map(({ name, path }, i) => (
              <Link
                ...
                to={path}
                onClick={scrollToTop}
                className="block px-6 py-3 text-base hover:bg-gray-100 whitespace-nowrap"
              >
                {name}
              </Link>
            ))}
